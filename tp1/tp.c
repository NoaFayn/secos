...
void create_gdt()
{
  ...
  // TODO(noa): Ensure that the size of the RAM is greater than the size of the GDT
  if (gdt.addr + gdt.limit < g_ram->addr + g_ram->len)
  {
    debug("Error: available space is not enough to store GDT data\n");
    return;
  }
...
void question3()
{
  ...
  gdt_reg_t gdt;
  get_gdtr(gdt);
  seg_desc_t * seg = &(((seg_desc_t *) gdt.addr)[3]);
  seg->type = SEG_DESC_DATA_RW;
  seg->g = 0;
  set_seg_base(seg, 0x600000);
  set_seg_limit(seg, 32);
  ...
  dump_mem(0x600000, 64);

  /*
   * Actual writing is done as address ES + dst.
   * ES = 0x600000; dst = 0x0
   */
  _memcpy8(dst, src, 32);
  debug("dest addr=0x%x\n", dst);
  dump_mem((int) dst, 64);

  dump_mem(0x600000, 64);

  /*
   * Trying to copy more than 32 bytes generates an error.
   */
  /*
  _memcpy8(dst, src, 64);
  debug("dest addr=0x%x\n", dst);
